...
# 사이트에서 실시간 데이터를 수신: QThread(백그라운드)
...
class MyWindow(QMainWindow):
    def __init__(self):
        ...
        # thread 
        self.worker = Worker()
        # callback함수: 데이터를 수신하면 호출될 함수: self.get_price()
        ...
        # 판다스 시리즈 객체 생성: 수신된 데이터를 판다스 시리즈 객체로 보관
        self.ticks = Series(dtype='float64')

        # window size
        self.setMinimumSize(800, 400)
        df = pyupbit.get_ohlcv(COIN, interval='minute1', count=80) # 분당 80회 데이터 수신

        # data  # candlechart = 막대그래프
        ...
        for index in df.index:
            ...
            high = df.loc[index, 'high']  # 상한가
            low = df.loc[index, 'low']    # 하한가
